...
export const getList = (token) => {
	return async (dispatch) => {
		...
		if(!response) {
			dispatch(fetchListFailed("There was an error with the connection. Fetching shoppinglist failed!"));
			return;
		}
		if(response.ok) {
...
export const addItem = (token,item) => {
	return async (dispatch) => {
		...
		dispatch(stopLoading());
		if(!response) {
			dispatch(addItemFailed("There was an error with the connection. Add new item failed"));
			return;
		}
		if(response.ok) {
...
export const removeItem = (token,id) => {
	return async (dispatch) => {
		...
		dispatch(stopLoading());
		if(!response) {
			dispatch(removeItemFailed("There was an error with the connection. Remove item failed"));
			return;
		}
		if(response.ok) {
...
export const edit = (token,item) => {
	return async (dispatch) => {
		...
		dispatch(stopLoading());
		if(!response) {
			dispatch(editItemFailed("There was an error with the connection. Edit item failed"));
			return;
		}
		if(response.ok) {
